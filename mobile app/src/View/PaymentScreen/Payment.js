...
import { StyleSheet, View, Text, Image, ScrollView, TouchableOpacity, ActivityIndicator, AsyncStorage } from 'react-native';
...
class Payment extends Component {
	...
	getCode = () => {
		...
		leadsRef.once('value', function (snapshot) {
			...
			arr.map((el) => {
				if (el.id == currentID) {
					me.setState({ currentUser: el, holder: el.firstName })
	...
	componentWillReceiveProps(newProps) {
		...
		AsyncStorage.getItem('UserFullData').then(a => {
			console.warn(a, 'user full data')
			this.setState({ holder: a.firstName, currentUser: a })
			// console.warn(a, "check async props")
		})
	...
	componentDidMount() {
		this.setState({ holder: this.props.navigation.state.params.firstName })
		this.getCode();
		AsyncStorage.getItem('language').then(a => {
	...
	// }
	onSubmit = async () => {
		const { holder, currentUser, asyncLanguage } = this.state
		// console.warn(currentUser, "AuthPaymentUserdata")
		...
		try {
			...
			axios
				.post('http://www.gimonii.com/api/doPayment', body, { headers })
				.then((res) => {
					// debugger
					AsyncStorage.getItem('userData').then((uid) => {
						...
						fireBase.database().ref('Users/' + uid).update({ 
							// paymentStatus: 1,
							 expireSubscriptionOn: afterOneMonthDate });
						// alert('uploaded');
						// console.log('url', url);
						this.setState({ loading: false });
						this.props.navigation.navigate('HomeInner', { payment: true })
					});
			...
			// else if(error.message == "Cannot read property 'firstName' of null"){
			// 	alert(asyncLanguage == "fr" ? French.ERROR_MISSING_CARD_NUMBER : 'English.ERROR_MISSING_CARD_NUMBER22222')
			// }
		}
	...
	render() {
		console.warn(this.props.navigation.state.params, 'params')
		...
		return (
			<View style={{ backgroundColor: '#f1f1f1', width: '100%', justifyContent: "space-between", flexDirection: 'column', alignItems: 'center', height: '100%' }}>
				...
				<TouchableOpacity style={{ height: 50, width: '90%', marginBottom: 20 }} onPress={() => this.onSubmit()}>
					...
					>
						<Text style={{ color: '#fff', fontSize: 16, lineHeight: 19, fontFamily: 'SFProDisplay-Regular' }}>{asyncLanguage == 'fr' ? French.ADD_CARD : English.ADD_CARD}</Text>
					</LinearGradient>
				</TouchableOpacity>
			</View>
		);
